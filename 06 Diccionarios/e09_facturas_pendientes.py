# Escribir un programa que gestione las facturas pendientes de cobro
# de una empresa. Las facturas se almacenarán en un diccionario donde
# la clave de cada factura será el número de factura y el valor el
# coste de la factura. El programa debe preguntar al usuario si quiere
# añadir una nueva factura, pagar una existente o terminar. Si desea
# añadir una nueva factura se preguntará por el número de factura y
# su coste y se añadirá al diccionario. Si se desea pagar una factura
# se preguntará por el número de factura y se eliminará del diccionario.
# Después de cada operación el programa debe mostrar por pantalla la
# cantidad cobrada hasta el momento y la cantidad pendiente de cobro.

""" Facturas pendientes

Entradas:
facturas (dict): Diccionario de facturas.
agregar (str): condicional para agregar facturas.
factura (str): llave del diccionario.
valor (str): valor del diccionario.
pagar (str): condicional para pagar facturas.
acumulado (int): acumulado de facturas pagadas.

Salidas:
(str): print con facturas pendientes por pagar y pagos realizados """

facturas = {}
agregar = 'si'

while agregar == 'si':
    factura = input('Ingrese el número de la factura:\n')
    valor = int(input(f'Ingrese el valor de la factura {factura}:\n'))
    facturas[factura] = valor
    agregar = input('Desea agregar más facturas? (si/no):\n')
print(facturas)
print(f'El total a pagar es de: {sum(facturas.values())}')
pagar = input('Desea pagar facturas? (si/no):\n')

acumulado = 0
while pagar == 'si':
    factura = input('Qué factura desea pagar?:\n')
    pago = facturas.pop(factura)
    acumulado += pago
    pagar = input('Desea pagar más facturas? (si/no):\n')


print(facturas)
print(f'El nuevo total a pagar es de: {sum(facturas.values())}')
print(f'El total de facturas pagadas fue de: {acumulado}')

# facturas = {}
# cobrado = 0
# pendiente = 0
# more = ''
# while more != 'T':
#     if more == 'A':
#         clave = input('Introduce el número de la factura: ')
#         coste = float(input('Introduce el coste de la factura: '))
#         facturas[clave] = coste
#         pendiente += coste
#     if more == 'P':
#         clave = input('Introduce el número de la factura a pagar: ')
#         coste = facturas.pop(clave, 0)
#         cobrado += coste
#         pendiente -= coste
#     print('Recaudado:', cobrado)
#     print('Pendiente de cobro: ', pendiente)
#     more = input('¿Quieres añadir una nueva factura (A), pagarla (P) o terminar (T)? ')
